...
As this is a application based reseach we have done lot of customization and developmet and amendment to cater our need.

\subsection{Applcation Design}

I desined lane violation as in below stratergy
detect lane separatly.
detect vehicle seperatly.

add boudry box in vehicle (lower base with adjustable width center to middle)

this is define as lane crossing threshould for vehicle.
( this adjustable and after some tuneup we come up to this width of the optimal boudr box)

For the helmet violation detection I use sepearate yolo model and call that method seperatly.


\subsubsection{Lane detection}

The applicattion succsss is highly deped on the lane detection method we have done intensive liitrure review on the subject

thanks to modern autonomous driving technnology advancement lande detection is widely updating and highly active area of reaserch.



After we finalizing the lande detection apporoch as LvLane.
We have trainned the model to cater sri lankan road.

initail setup doen with RTX 600 powerfull trainin model.

At the time I have a sinlgle cpu machine so  I have ot covert that into cpu mode.
later on broght GPU machine but have not enogh time to convert back into GPU mode as current machine supprt more latest model.
not supported for recent model.


\subsubsection(perfocrmance test)
.

 
\paragraph{camera setup}
(here need to describe LVLane camera setup detaisl and reference document)


Anotations and camera adjustment encounted problems.
camera need to be 120 apacher and LV lane trainng does not hit the target.

LVLane camera setup (there wear lot of camera they have used but fortunate to new camera desing we were able to 
replicate it with multi camera setp with mobile phone.). near to tha


\subsubsection{Data prparation for lane detection}

capture videos
and ffimg to identify key frames.

anotaed with imglable tools.

( described annoated tools.)

\subsection{Helmet detection details}

\subsubsection{Data preparation}
We also alble to get some times for helmet violation deteaction.
gotten baseline dataset.
pre annoted .
we alson need to do some fine tune.

\subsubsection{trarinnig}

we Initially use Yolo n model but it not gibin avvolct pergformance

then move to x model and it generate good perfoamcen


\subsection{Inegrated system}

as full product we use lanve vilation detection with helmet violation detection as full product.
...
If could use many camera footages we can more generalize the trainned model.
Due to time constrain I am only use one camera.

For data collection stratergy could improve when raining condition.
Night light conditions

some huristinc can be improve like if near the pedestrial crossing and heavy bend we can ask model to consider that aspects as well to more acuratly guessing lane 
(near pedestrial crossing and hevey bend most probabaly have solid lanes)
